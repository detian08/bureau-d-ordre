...
class reglement_paiement(osv.osv):
    ...
    _columns = {
        'code_reglement': fields.char('Reference', size=100,readonly=True),
    ...
    def reg_cancel(self, cr, uid, ids, context=None):
    ...
    def reg_reclose(self, cr, uid, ids, context=None):
...
WHERE 
  ...
  ORDER BY montant_rest asc
		...
		var=0.0
		print "rd.montant00000000000000000",rd.montant
		montant_rest=line['montant_rest']
		print "montant_rest1111111111111",montant_rest
		#montant_paye=line['montant_paye']
		montant_paye=round(line['montant_paye'],3)
		...
		#var=ttc-(round(montant_paye,3)+rd.montant)
...
WHERE 
  reglement_detail_impaye.piece_id = reglement_piece.id and reglement_piece.id=%s and reglement_detail_impaye.id =%s
	...
	transaction_exite=obj_bank_stat_line.search(cr, uid, [('ref','=',reglement.code_reglement)])
	if transaction_exite:
		raise osv.except_osv(_('Transaction déjà faite!'), _('Vous avez dejà passé ce reglement à la caisse'))
	montant=reglement.montant_espece / reglement.currency_id.rate
	val = {
...
class reglement_detail_impaye(osv.osv):
    ...
    def	piece_id_change(self, cr, uid, ids, piece_id,parent_id, context=None):
	res_final = {}
	result = {}
	if piece_id:
		
	    print "piece_id**********************",piece_id
	    piece_ids = self.pool.get('reglement.piece').search(cr, uid, [('id','=',piece_id)])	
	    print "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",piece_ids
...
where ( r.id = %d )
    ...
    def validation_retenu(self, cr, uid, ids, context=None):
            ...
            for retenue_src in self.browse(cr, uid, ids, context=context):
                ...
                if (type_retenu == 'TTC') and (totale_ttc < montant_retenu) and (designation==0.015):
