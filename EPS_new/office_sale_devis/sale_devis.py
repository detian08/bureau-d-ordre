...
class product_template(osv.Model):
    ...
    def _devis_count(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for template in self.browse(cr, uid, ids, context=context):
            res[template.id] = sum([p.devis_count for p in template.product_variant_ids])
        return res

    _columns = {

        'devis_count': fields.function(_devis_count, string='# Purchases', type='integer'),
    }



    def action_view_devis(self, cr, uid, ids, context=None):
        products = self._get_products(cr, uid, ids, context=context)
        result = self._get_act_window_dict(cr, uid, 'office_sale_devis.action_sale_devis_line_product_tree', context=context)
...
class product_product(osv.Model):
    ...
    def _devis_count(self, cr, uid, ids, field_name, arg, context=None):
        Purchase = self.pool['sale.devis']
        return {
            product_id: Purchase.search_count(cr,uid, [('devis_lines.product_id', '=', product_id)], context=context) 
    ...
    def action_view_devis(self, cr, uid, ids, context=None):
        if isinstance(ids, (int, long)):
            ids = [ids]
        result = self.pool['product.template']._get_act_window_dict(cr, uid, 'office_sale_devis.action_sale_devis_line_product_tree', context=context)
