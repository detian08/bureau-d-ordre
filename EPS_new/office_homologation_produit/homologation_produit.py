...
#salwa ksila update 22/03/217
    @api.one
    @api.depends('taux_divers_euro','taux')
    def _get_total(self):
		for r in self:
			r.taux_divers_dt = r.taux_divers_euro * r.taux
    ...
    _columns = {
        ...
        'marge_max': fields.many2one('marge.produit','Marge Max',required=True, digits_compute= dp.get_precision('Product Price'),readonly=True,states={'draft':[('readonly',False)]}),
        'marge_min': fields.many2one('marge.produit','Marge Min',required=True, digits_compute= dp.get_precision('Product Price'), readonly=True,states={'draft':[('readonly',False)]}),
        'amount_invoice_euro': fields.float('Montant Facture €', readonly=True, digits_compute= dp.get_precision('Product Price')),
        'amount_invoice_DT': fields.float('Montant Facture DT', readonly=True, digits_compute= dp.get_precision('Product Price')),
        'homologation_lines': fields.one2many('homologation.produit.line', 'homologation_id', ' ',readonly=True,states={'draft':[('readonly',False)]}),        	'amount_total_euro':fields.float(string='Total €', digits_compute= dp.get_precision('Product Price'),
        store=True, readonly=True, compute='_amount_all', track_visibility='always'),
	    'amount_total_dt':fields.float(string='Total DT', digits_compute= dp.get_precision('Product Price'),
         store=True, readonly=True, compute='_amount_all', track_visibility='always'),
        'charges_homologation_lines': fields.one2many('charge.homologation.produit.line', 'homologation_id', ' '),       
	    'amount_total_chrage_dt':fields.float(string='Total Charges DT', digits_compute= dp.get_precision('Product Price'),
        store=True, readonly=True, compute='_amount_all', track_visibility='always'),
        'categ_lines': fields.one2many('homologation.produit.categ', 'homologation_id'," "),   
	    'amount_total_DD':fields.float(string='Total DD DT', digits_compute= dp.get_precision('Product Price'),
        store=True, readonly=True, compute='_amount_all', track_visibility='always'),
	   'taux_divers_euro': fields.float('Taux divers €'),
	   'taux_divers_dt': fields.float('Taux divers', digits_compute=dp.get_precision('Product Price'),
									   store=True, readonly=True, compute='_get_total', track_visibility='always'),
			...
			for categ in categ_obj:
				...
				if import_lines:
					...
					for line in import_lines:
						...
						taux_DD=round((line.price_subtotal_dt / montant_total_categ),4)
						print "taux_DDtaux_DDtaux_DDtaux_DDtaux_DD",taux_DD
    ...
    def button_add_invoice(self, cr, uid, ids, context=None):
		...
		##ligne facture
		imporation_obj=self.browse(cr, uid, ids)
		if imporation_obj.state not in ['draft']:
			raise osv.except_osv(('Homologation Confirmé'), ('Vous ne pouvez pas initialiser l homologation, vous pouvez utiliser le button retour au brouillon pour le faire '))
		if not imporation_obj.taux :
			raise osv.except_osv(('Taux erroné'), ('Veuillez mentionner le Taux '))
		if imporation_obj.homologation_lines:
			for line in imporation_obj.homologation_lines:
				self.pool.get('homologation.produit.line').unlink(cr, uid, line.id)
		if imporation_obj.charges_homologation_lines:
			for line in imporation_obj.charges_homologation_lines:
				self.pool.get('charge.homologation.produit.line').unlink(cr, uid, line.id)
		...
		X=imporation_obj.amount_invoice_euro - imporation_obj.taux_divers_euro



		for line in invoice_lines_obj:
			#get the id from product.product
			product_tmp=self.pool.get('product.product').browse(cr, uid,line.product_id.id).product_tmpl_id.id
			#get the id of product_tmp_id from product.template
			product=self.pool.get('product.template').browse(cr, uid,product_tmp)
			#if product.categ_douane.id:
			val={
				'product_id':line.product_id.id,
				'categ_id':product.categ_douane_id.id,
				'designation':line.name,
				'price_unit':line.price_unit,
				'product_qty':line.quantity,
				'price_subtotal_euro':line.price_subtotal,
				#salwa ksila update23/07/2017
				'price_subtotal_dt': line.price_subtotal  * imporation_obj.taux,
				#'price_subtotal_dt': (line.price_subtotal + imporation_obj.taux_divers_euro)  * imporation_obj.taux,
				#'taux_FF':(line.price_subtotal * imporation_obj.taux)/imporation_obj.amount_invoice_DT,
				'taux_FF':(line.price_subtotal / X),
				'homologation_id':ids[0],
...
class homologation_produit_line(osv.osv):
    ...
    @api.one
    @api.depends('price_unit','product_qty','product_id', 'homologation_id.taux','homologation_id.categ_lines.total' ,'homologation_id.taux_divers_euro')#, 'devis_id.currency_id')'homologation_id.amount_total_chrage_dt'
    def _amount_line(self):
	    ...
	    X = self.homologation_id.amount_invoice_euro-self.homologation_id.taux_divers_euro
	    print "montant deduc XXXXX==============", X
	    print "prix euro=====", self.price_subtotal_euro
	    print "ancien %FF=======", self.taux_FF
	    self.taux_FF = self.price_subtotal_euro / X
	    print "NOUVEAU %FF=======", self.taux_FF
	    self.taux_divers = self.homologation_id.taux_divers_euro* self.taux_FF


		#salwa ksila update 23/03/2017
	    #self.price_subtotal_dt = (self.price_subtotal_euro ) * self.homologation_id.taux
	    #self.price_subtotal_dt = (self.price_subtotal_euro + self.homologation_id.taux_divers_euro) * self.homologation_id.taux
	    print "ancien TOTAL ARTICLE DINAR=======", self.price_subtotal_dt
	    self.price_subtotal_dt = (self.price_subtotal_euro + self.taux_divers) * self.homologation_id.taux
	    print "NOUVEAU TOTAL ARTICLE DINAR=======", self.price_subtotal_dt
	    total=0.0
	    ...
	    self.DD=round((self.taux_DD*total),4)
 ...
 #salwa KSILA 23/03/2017
    @api.one
    @api.depends('taux_FF','homologation_id.taux_divers_euro')
    def _calcul_taux_divers(self):
	self.taux_divers = self.homologation_id.taux_divers_euro* self.taux_FF
    ...
    _columns = {
        ...
        'categ_id' : fields.many2one('product.category.douane' , 'NGP' , related="product_id.categ_douane_id", readonly=True ),#
        'product_id': fields.many2one('product.product', 'Produit', ondelete='restrict',),
        'designation': fields.char('Désignation ',),
        'taux_FF': fields.float('% FF', digits_compute= dp.get_precision('Product Price'), compute="_amount_line",),
        'taux_DD': fields.float('% DD', digits_compute= dp.get_precision('Account'),),
        'FF': fields.float('F Fixes', digits_compute= dp.get_precision('Product Price'),),#compute='_amount_FF',
        'DD': fields.float(string='DD', digits_compute= dp.get_precision('Account'),store=True,),#compute='_amount_DD'
        ...
        'price_subtotal_dt': fields.float('Prix Total DT', digits_compute= dp.get_precision('Product Price'),store=True, readonly=True, compute='_amount_line'),
        'price_subtotal_revient_dt': fields.float('Prix Total Revient', digits_compute= dp.get_precision('Product Price'),store=True, readonly=True, compute='_amount_line'),
        'price_unit_revient_dt': fields.float('Prix U revient', digits_compute= dp.get_precision('Product Price'), readonly=True, compute='_amount_line'),
        'marge_min': fields.float('Marge Min', digits_compute= dp.get_precision('Product Price'), readonly=True, compute='_amount_line'),
        'marge_max': fields.float('Marge Max', digits_compute= dp.get_precision('Product Price'), readonly=True, compute='_amount_line'),
        'coeff_rev': fields.float('Coeff rev', readonly=True, compute='_amount_line'),#,digits_compute= dp.get_precision('Product Price')
	...
	'taux_divers': fields.float(string='Taux divers €', digits_compute=dp.get_precision('Account'), readonly=True, compute="_amount_line"),
	...
	res_final = {}
        if product:
            ...
            res_final = {'value':{'designation':designation ,'categ_id':product_obj.categ_douane_id.id}}#
...
class homologation_produit_categ(osv.osv):
    ...
    _columns = {
        ...
        'taux_1' : fields.char('zzu' ,required=True,),
        'taux_2' : fields.char('znt' ,required=True,),
        'taux_3' : fields.char('qst' ,required=True,),
        'taux_4' : fields.char('uzc' ,required=True,),
        'taux_5' : fields.float('AIR' ,digits_compute= dp.get_precision('Product Price')),
        'total' : fields.float(string='Total', digits_compute= dp.get_precision('Product Price')),
...
class marge_produit(osv.osv):
    ...
    _columns = {
    	'name' : fields.char('Name' ,required=True),
    	'taux' : fields.float('Taux ' ,required=True,digits_compute= dp.get_precision('Product Price')),
    }
...
class charge_homologation_produit_line(osv.osv):
    ...
    _columns = {
        ...
        'amount_charge': fields.float('Montant', digits_compute= dp.get_precision('Product Price')),
