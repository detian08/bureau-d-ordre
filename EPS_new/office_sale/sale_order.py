...
class sale_order_line(osv.osv):
	...
	###Remises Fournisseurs et Total de la commande
	...
	def product_id_change(self, cr, uid, ids, pricelist, product, qty=0, uom=False, qty_uos=0, uos=False, name='', partner_id=False, lang=False, update_tax=True, date_order=False, packaging=False, fiscal_position=False, flag=False, context=None):
		...
		if (not uom) and (not uos):
		    ...
		    domain = {'product_uom':[('category_id', '=', product_obj.uom_id.category_id.id)],'product_uos':[('category_id', '=', uos_category_id)]}
		...
		if not pricelist:
			...
			if price is False:
				...
				##get the min of price and list_price
				# base_url = self.pool.get('ir.config_parameter').get_param('web.base.url')
				# print base_url

				#####définir context


				for line in self.pool.get('sale.order.line').search(cr, uid, [('product_id','=',product)]):
					order_id = self.pool.get('sale.order.line').browse(cr, uid, line).order_id.id
					product_tmpl_id = self.pool.get('product.product').browse(cr, uid, product).product_tmpl_id.id
					price_web = self.pool.get('product.template').browse(cr, uid, product_tmpl_id, context).price
					origine = self.pool.get('sale.order').browse(cr, uid, order_id).origine

					if origine == 'front':
						result.update({'price_unit': min(price, price_web)})
					else :
						result.update({'price_unit':price})

					partner_id = self.pool.get('sale.order').browse(cr, uid, order_id).partner_id.id
					revendeur = self.pool.get('res.partner').browse(cr, uid, partner_id).revendeur

					product_brand_id = self.pool.get('product.template').browse(cr, uid,product_tmpl_id).product_brand_id.id
					name = self.pool.get("product.brand").browse(cr, uid, product_brand_id).name
					undiscount_total = self.pool.get('sale.order').browse(cr, uid, order_id).undiscount_total
					###### s'il n'est pas Revendeur compte publique (admin) ou compte client
					if name in ("LAERDAL", "SPENCER", "AMIITALIA") and undiscount_total < 5000.0 and revendeur == False:
						result.update({'discount': 10})
					if name in ("LAERDAL", "SPENCER", "AMIITALIA") and (undiscount_total > 5000.0 and undiscount_total < 10000.0) and revendeur == False:

						result.update({'discount': 15})
					if name in ("LAERDAL", "SPENCER", "AMIITALIA") and (undiscount_total > 10000.0 and undiscount_total < 20000.0) and revendeur == False:

						result.update({'discount': 20})
					if name in ("LAERDAL", "SPENCER", "AMIITALIA") and undiscount_total > 20000.0 and revendeur == False:

						result.update({'discount': 25})
					if name in ("DUMONT SECURITE", "LOCAL", "VIMETESCA") and undiscount_total < 5000.0 and revendeur == False:

						result.update({'discount': 5})
					if name in ("DUMONT SECURITE", "LOCAL", "VIMETESCA") and undiscount_total > 5000.0 and revendeur == False:

						result.update({'discount': 10})

					###### s'il est Revendeur et compte client
					if name in ("LAERDAL", "SPENCER", "AMIITALIA") and undiscount_total < 5000.0 and revendeur == True:
						result.update({'discount': 15})
					if name in ("LAERDAL", "SPENCER", "AMIITALIA") and (undiscount_total > 5000.0 and undiscount_total < 10000.0) and revendeur == True:
						result.update({'discount': 20})
					if name in ("LAERDAL", "SPENCER", "AMIITALIA") and undiscount_total > 10000.0 and revendeur == True:
						result.update({'discount': 25})

					if name in ("DUMONT SECURITE", "LOCAL", "VIMETESCA") and undiscount_total < 5000.0 and revendeur == True:
						result.update({'discount': 10})
					if name in ("DUMONT SECURITE", "LOCAL", "VIMETESCA") and undiscount_total > 5000.0 and revendeur == True:
						result.update({'discount': 15})
...
class sale_order(osv.osv):
	...
	def create(self, cr, uid, vals, context=None):
		menuitem = context.get('menuitem')
		order_id = super(sale_order, self).create(cr, uid, vals, context=context)
		if menuitem :
			self.write(cr,uid,order_id,{'origine':'back'})
		return order_id
	...
	def _amount_line(self):
		...
		for line in self.order_line :
			...
			discount = self.env['sale.order.line'].browse(line.id).discount


		self.discount_total = sum( (line.product_uom_qty * line.price_unit * ((discount or 0.0)/100.0) ) for line in self.order_line )


		self.undiscount_total = sum(line.product_uom_qty * line.price_unit for line in self.order_line)
        
	_columns = {
		'origine' : fields.selection([('front', 'Front'),('back', 'Back')]),
	...
	def returntodraft(self, cr, uid, ids,context=None):
		...
		#print "order_line.order_id.id",order_line.id
		#print "aaaaaaaaaaaaaaaaaaa",ids[0]
		#self.pool.get('sale.order.line').write(cr, uid, ids[0], {'state': 'draft'})
		#self.pool.get('sale.order').write(cr, uid, order_line.order_id.id, {'state': 'draft'})		#print "apres write ******"
	...
	def returntodone(self, cr, uid, ids,context=None):
		for order in self.browse(cr, uid, ids, context=context):
			for line in order.order_line:
				self.pool.get('sale.order.line').write(cr,uid,[line.id],{ 'state':'confirmed'})
