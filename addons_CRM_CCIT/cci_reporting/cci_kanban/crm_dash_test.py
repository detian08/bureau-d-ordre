...
###############################################""""""
		if list_prod and user_id == 1:
			...
			self.env.cr.execute('SELECT COUNT(c.id), default_code, p.id FROM crm_lead c, product_product p where c.product_id = p.product_tmpl_id AND c.stage_id NOT IN %s AND product_id IN %s GROUP BY (default_code,p.id)',((6,7),tuple(list_prod)))


###select count(c.id), default_code, p.id from crm_lead c, product_product p where c.product_id = p.product_tmpl_id and c.stage_id not in (6,7) group by(default_code,p.id);
...
###########################################################""
		...
		if list_prod and user_id == 1:
			...
			self.env.cr.execute("SELECT COUNT(c.id), default_code, p.id FROM crm_lead c, product_product p where c.type_act != '' AND c.product_id = p.product_tmpl_id AND c.stage_id NOT IN %s AND product_id IN %s GROUP BY (default_code,p.id)",((6,7),tuple(list_prod)))
	...
	list_section = []
 	if self.type == 'product' :
		...
		if list_prod and user_id == 1:
			...
			self.env.cr.execute('SELECT SUM(c.planned_revenue), default_code, p.id FROM crm_lead c, product_product p WHERE c.product_id = p.product_tmpl_id AND c.stage_id NOT IN %s AND product_id IN %s GROUP BY (default_code,p.id)',((6,7),tuple(list_prod)))
