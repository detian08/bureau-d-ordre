...
#self.env.cr.execute('SELECT DISTINCT COUNT(distinct (c.id)), default_code FROM crm_lead c, product_product p WHERE c.stage_id NOT IN %s AND c.product_id IN %s AND p.product_tmpl_id IN %s GROUP BY default_code',((6,7),tuple(list_prod),tuple(list_prod)))
    @api.multi
	...
	list_section_comm = []
 	if self.type == 'opportunity' :###type de la carte 
		...
		##Liste des produits à suivre et validé
		product_ids = self.env['product.template'].search([('a_suivre','=',True),('state','=','validate')])
		if product_ids and user_id == 1:
			for product in product_ids :
				default_code = self.env['product.product'].search([('product_tmpl_id','=',product.id)]).default_code
				self.env.cr.execute('SELECT COUNT(id) FROM crm_lead WHERE stage_id NOT IN %s AND product_id = %s',((6,7),product.id))
				res1 = self.env.cr.dictfetchall()
				for j in res1:
					value=j.get('count')
					data.append({'label':default_code , 'value':value ,'id':self.id})
...
###########################################################""
		...
		if list_prod and user_id == 1:
			...
			query5 = "SELECT default_code FROM product_product"
	...
	list_section = []
 	if self.type == 'product' :
		...
		if list_prod and user_id == 1:
			...
			self.env.cr.execute('SELECT SUM(c.planned_revenue), default_code FROM crm_lead c, product_product p WHERE c.stage_id NOT IN %s AND c.product_id IN %s AND p.product_tmpl_id IN %s GROUP BY default_code',((6,7),tuple(list_prod),tuple(list_prod)))
			admin = self.env.cr.dictfetchall()
			print 'res req revenu espere:',admin
			for i in admin:
				label=i['default_code']
				price=i['sum']
