	...
	value = 0
 	if self.type_dash == 'state' :
		...
		if user_id == 1 : #pour l'administrateur
			...
			if list_prod :#pour chaque opportunité tt les états 
				#self.env.cr.execute("SELECT COUNT(id), stage_id FROM crm_lead GROUP BY stage_id")
 	...
 	if self.type_dash == 'ag' :
		...
		if user_id == 1 : #pour l'administrateur
			...
			for product in product_ids :
				list_prod.append(product.id)
			if list_prod:
				self.env.cr.execute("SELECT COUNT(id), user_id FROM crm_lead GROUP BY user_id")##par commerciale group by user_id
				res = self.env.cr.dictfetchall()
			        print 'res.....',res
				for i in res:
					user_id = i['user_id']
