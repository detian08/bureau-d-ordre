...
class crm_dashboard_opportunity(models.Model):
    ...
    @api.one
    def _dashboard_ca_ag_perdu(self):
		self.dashboard_ca_ag_perdu = json.dumps(self.get_dashboard_ca_ag_perdu())
    @api.one
    ...
    dashboard_ca_ag_perdu = fields.Text(compute='_dashboard_ca_ag_perdu')
...
################################chiffre d'affaires à l'état gagné########################
    @api.multi
	...
	value = 0
 	...
 	if self.type_dash == 'chiffregagne' :
		user_id = self.env['res.users'].browse(self.env.uid).id
		##Liste des produits à suivre
		product_ids = self.env['product.template'].search([('a_suivre','=',True)])
...
################################chiffre d'affaires à l'état perdu########################
    @api.multi
    def get_dashboard_ca_ag_perdu(self):
	data = []
	list_prod = []
	list_section = []
	value = 0

 	if self.type_dash == 'chiffreperdu' :
		user_id = self.env['res.users'].browse(self.env.uid).id
		##Liste des produits à suivre
		product_ids = self.env['product.template'].search([('a_suivre','=',True)])
		for product in product_ids :
		...
		if list_prod :
			if user_id == 1 : #pour l'administrateur
				section_ids = self.env['crm.case.section'].search([])
				for section in section_ids:
					self.env.cr.execute("SELECT SUM(planned_revenue), section_id FROM crm_lead WHERE product_id in %s AND stage_id =%s AND section_id =%s GROUP BY section_id",(tuple(list_prod),7,section.id))
					res = self.env.cr.dictfetchall()
                                        print 'res.....',res
					for i in res:
						value = i.get('sum')
