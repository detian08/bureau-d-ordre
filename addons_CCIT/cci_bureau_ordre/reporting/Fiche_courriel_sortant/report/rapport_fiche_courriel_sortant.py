	...
	def generate_records(self, cr, uid, ids, data, context):
		...
		if 'form' in data:
			...
			if permanent == True:
				...
				print ce_ids
				for ce in ce_ids:
					ref = self.pool.get('cci.courriel.sortant').browse(cr, uid, ce, context=context).reference
...
#######Etat#####
			...
			if state : 
				etat_ids = pool.get('cci.courriel.sortant').search(cr, uid, [('state','=',state)])
				print 'etat_ids..............',etat_ids
				for etat in etat_ids:
					print 'etat..............',etat
					ref = self.pool.get('cci.courriel.sortant').browse(cr, uid, etat, context=context).reference
					print 'ref..............',ref
					#date_courriel = self.pool.get('cci.courriel.entrant').browse(cr, uid, etat, context=context).date_courriel
					note_id = self.pool.get('cci.courriel.sortant').browse(cr, uid, etat, context=context).note_id
					print 'note_id..............',note_id
					type_id = self.pool.get('cci.courriel.sortant').browse(cr, uid, etat, context=context).type_id.name
					print 'type_id..............',type_id
					objet = self.pool.get('cci.courriel.sortant').browse(cr, uid, etat, context=context).objet
					print 'objet..............',objet
					name = self.pool.get('cci.courriel.sortant').browse(cr, uid, etat, context=context).name
					print 'name..............',name
					dept_id = self.pool.get('cci.courriel.sortant').browse(cr, uid, etat, context=context).dept_id.name
