...
#################"function compute
	...
	def to_soumpre(self):
		...
		if present == True :
			...
			mail_vals = {
				'body': '<html>Merci de traiter un courriel entrant soumis.</html>',
				'record_name': "Traitement d'un courriel entrant",
				'res_id': self.id,
				'reply_to': user_name,
				'author_id': uid,
...
##################"
	...
	#def to_assistantpre(self):
		#print "hello Soumis au vice-président"
...
####traiter#####
		...
		traiter_ids = self.env['cci.courriel.entrant'].browse(self.id).dept_id.id
		print 'traiter_ids....',traiter_ids
...
#####Notification conseiller####
		uid_email = self.env['res.users'].browse(uid).email
		conseiller_ids = self.env['cci.courriel.entrant'].browse(self.id).dept_id.id
