...
class Wizard(models.TransientModel):
    _name = 'cci.document.alfresco.product.wizard'
    ...
    def upload_document(self):
        ...
        try:
            Products = repo.getObjectByPath('/CRM/Products')
        except:
            Products = crm.createFolder('Products')
        try:
            sFolder = repo.getObjectByPath( '/CRM/Products/' +self._get_cin(self.env.context))
        except:
            sFolder = Products.createFolder(self._get_cin(self.env.context) )
        ...
        record = self.env['cci.document.alfresco.produit'].create({'node' :document.id ,'nom_fichier':
            self.nom_fichier ,'description' :self.description ,'produit_id' :self._get_active_id(self.env.context) ,})
